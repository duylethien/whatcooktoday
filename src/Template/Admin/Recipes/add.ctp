<div class="page-title">
    <div class="breadcrumb-env" style="float: left">
        ...
        <ol class="breadcrumb bc-1" >
            ...
            <li class="active">
                <strong><?php echo ('Add Recipe') ?></strong>
            </li>
        </ol>
    </div>
</div>
...
<div class="panel panel-default">
    <div class="panel-heading">
        <h3 class="panel-title"><?php echo ('Add Recipe') ?></h3>
        <div class="panel-options">
        </div>
    </div>
    <div class="panel-body">
        <?php
        if (empty($recipes['status']) && empty($recipes['difficulty'])): $recipes['status'] = 1 && $recipes['difficulty'] = 1; endif;
        echo $this->Flash->render();

        $this->Form->setTemplates([
            'inputContainer' => '<div class="form-group{{required}} col-md-10"> {{content}} <span class="help">{{help}}</span></div>',
            'input' => '<input type="{{type}}" name="{{name}}" class="form-control form-control-danger" {{attrs}}/>',
            'inputContainerError' => '<div class="form-group has-danger {{type}}{{required}} col-md-10">{{content}}{{error}}</div>',
            'error' => '<div class="text-danger">{{content}}</div>',
            'textarea' => '<textarea  name="{{name}}" class="form-control col-md-10" {{attrs}}>{{value}}</textarea>',
        ]);
